...
async def process_day_summary(
    ...
    logger.info("day_summary_task_started checkin_id=%s", checkin_id)
    try:
        await processor.process_checkin(checkin_id=UUID(checkin_id))
    except Exception:
        logger.exception("day_summary_task_failed checkin_id=%s", checkin_id)
        raise
    logger.info("day_summary_task_finished checkin_id=%s", checkin_id)
